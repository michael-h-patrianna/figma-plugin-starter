...
import {
  ...
  hideProgressModal,
  hideProgressModalByOperationId,
  ProgressModalService,
  progressModalState,
  showProgressModal,
  updateProgressModal,
  updateProgressModalByOperationId
...
describe('ProgressModal Service', () => {
  ...
  describe('Main Thread Integration', () => {
    ...
    it('should create modal with operationId', () => {
      const id = showProgressModal({
        title: 'Test Modal',
        description: 'Test description',
      ...
      const modal = getProgressModal(id);
      expect(modal).toBeDefined();
      expect(modal!.operationId).toBe('test-operation-123');
    });

    it('should start operation and send message to main thread', () => {
      const result = ProgressModalService.startOperation(
        {
      ...
      expect(typeof result.modalId).toBe('string');
      expect(typeof result.operationId).toBe('string');

      // Check that modal was created with correct operationId
      const modal = getProgressModal(result.modalId);
      expect(modal).toBeDefined();
      expect(modal!.operationId).toBe(result.operationId);
      expect(modal!.title).toBe('Scanning Nodes');

      // Check that message was sent to main thread
      expect(mockSendToMain).toHaveBeenCalledTimes(1);
      expect(mockSendToMain).toHaveBeenCalledWith('SCAN_NODES', {
    ...
    it('should start operation with minimal options', () => {
      const result = ProgressModalService.startOperation(
        {
      ...
      // Check that message was sent with just operationId
      expect(mockSendToMain).toHaveBeenCalledWith('SIMPLE_OP', {
        operationId: result.operationId
      });
    });

    it('should update modal progress by operationId', () => {
      const { modalId, operationId } = ProgressModalService.startOperation(
        { title: 'Test Op', description: 'Testing...' },
        'TEST_OP'
      );

      // Update by operationId using the new function
      updateProgressModalByOperationId(operationId, 50, { description: 'Half done...' });

      const modal = getProgressModal(modalId);
      expect(modal!.progress).toBe(50);
      expect(modal!.description).toBe('Half done...');
    });

    it('should handle concurrent operations with different operationIds', () => {
      const op1 = ProgressModalService.startOperation(
        { title: 'Operation 1' },
        'OP1'
      );
      const op2 = ProgressModalService.startOperation(
        { title: 'Operation 2' },
        'OP2'
      );
      const op3 = ProgressModalService.startOperation(
        { title: 'Operation 3' },
        'OP3'
      );
      ...
      expect(op2.operationId).not.toBe(op3.operationId);
      expect(op1.operationId).not.toBe(op3.operationId);

      // All modals should exist
      expect(getProgressModal(op1.modalId)).toBeDefined();
      expect(getProgressModal(op2.modalId)).toBeDefined();
      expect(getProgressModal(op3.modalId)).toBeDefined();

      // Should have sent 3 different messages
      expect(mockSendToMain).toHaveBeenCalledTimes(3);
    });

    it('should hide modal by operationId', () => {
      const { modalId, operationId } = ProgressModalService.startOperation(
        { title: 'Test Operation', description: 'Testing hide by operationId' },
        'TEST_HIDE'
      );

      // Verify modal exists
      expect(getProgressModal(modalId)).toBeDefined();

      // Hide by operationId using the new function
      hideProgressModalByOperationId(operationId);

      // Verify modal is gone
      expect(getProgressModal(modalId)).toBeUndefined();
    });

    it('should handle hiding non-existent modal by operationId gracefully', () => {
      const consoleSpy = jest.spyOn(console, 'warn').mockImplementation();

      // Try to hide modal with non-existent operationId
      hideProgressModalByOperationId('non-existent-operation-id');

      expect(consoleSpy).toHaveBeenCalledWith('Progress modal with operationId "non-existent-operation-id" not found');
      consoleSpy.mockRestore();
    });
  });
